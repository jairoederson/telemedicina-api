...
$(function () {
  ...
  var tableEspecialidades = $("#tableEspecialidades").DataTable({
    language: {
    ...
    columns: [{ data: "id" }, { data: "name" }, { data: "description" },{ "data": "acciones" }],
  });
});
...
function listarEspecialidad() {
  $.ajax({
    ...
    .done(function (dataRpta) {
      ...
      $("#tableEspecialidades").DataTable().clear().draw();
      jQuery.each(dataRpta, function (i, val) {
          ...
          val.acciones =
            `
              ...
              <button title='Editar' onclick="location.href='/admin/actualizarespecialidad/` +
            val.id +
        ...
        $("#tableEspecialidades").DataTable().row.add(val).draw();
      });
    })
...
function eliminarEspecialidad(id) {
  $.ajax({
    ...
    dataType: "json",
    data: { idespecialidad: id, _token: token },
    beforeSend: function (xhr) {
      //alert(id);
    },
  })
