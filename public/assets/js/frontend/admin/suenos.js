...
$(function () {
  ...
  var tablesuenos = $("#tablesuenos").DataTable({
    language: {
...
function listarsuenos() {
  $.ajax({
    ...
    .done(function (dataRpta) {
      ...
      $("#tablesuenos").DataTable().clear().draw();
      jQuery.each(dataRpta, function (i, val) {
        val.acciones =
          `
                  <div class='text-center'>
                    
              <button title='Editar' onclick="location.href='/admin/actualizarsuenos/` +
          val.id +
        ...
        $("#tablesuenos").DataTable().row.add(val).draw();
      });
    })
...
function eliminarsuenos(id) {
  $.ajax({
    type: "post",
    url: eliminarsuenos,
    dataType: "json",
    data: { id_suenos: id, _token: token },
    beforeSend: function (xhr) {
      //alert(id);
    },
  })
