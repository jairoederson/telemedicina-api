<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="user-scalable=no, width=device-width, initial-scale=1.0" />
	<title>Bot&oacute;n de pago</title>
	<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0-rc.2/css/materialize.min.css">
	<style media="screen">
		body{
			background-color: #e22715;
			color: #fff;
		}
		.messageServer{
			font-size: 40px;
			text-align: center;
			padding-top: 200px;
		}
		.btn-green{
			background-color: #868686;
			text-align: center;
		}
	</style>
</head>
<body>
<div class="">
	<?php
	include "librerias/funciones.php";
	include 'librerias/lib.inc';
	if (isset($_POST['transactionToken'])){
		$sessionToken = recupera_sessionToken();
		$transactionToken = $_POST['transactionToken'];
		// echo "<pre>";
		// var_dump($_POST);
		// echo "<pre>";
		$respuesta = authorization("dev",MERCHANT,$transactionToken,AccessKey,SecretAccessKey,$sessionToken);
		if (version_compare(PHP_VERSION, '5.4.0', '<')) {
			$answer = json_decode($respuesta, true);
			if($answer["errorCode"] == 404){
				echo "<div class='messageServer'>Error 404 en el servidor de Visanet</div>";
				// echo "<div class='text-align: center'><a href='http://localhost:4200/nueva-consulta' class='btn btn-green'>Volver a Nueva Consulta Medica</a></div>";
				echo "<a href='https://www.alo.doctor/web/paciente/dist/nueva-consulta' class='btn'>Volver a Nueva Consulta Medica</a>";
			}else if($answer["errorCode"] == 500){
				echo "<div class='messageServer'>Error 500 en el servidor de Visanet</div>";
				// echo "<div class='text-align: center'><a href='http://localhost:4200/nueva-consulta' class='btn btn-green'>Volver a Nueva Consulta Medica</a></div>";
				echo "<a href='https://www.alo.doctor/web/paciente/dist/nueva-consulta' class='btn'>Volver a Nueva Consulta Medica</a>";
			}else if($answer["errorCode"] == 0){
				echo "<div class='messageServer'>Peticion exitosa, redirigiendo a pagina de llamada ...</div>";

				// header('Location: ' . "http://localhost:4200/nueva-consulta", true, false ? 301 : 302);
				header('Location: ' . "https://www.alo.doctor/web/paciente/dist/nueva-consulta", true, false ? 301 : 302);
    		exit();
			}

			echo "<pre>";
			echo jsonpp($respuesta);
			echo "<pre>";
		}else{
			$answer = json_decode($respuesta, true);
			if($answer["errorCode"] == 404){
				echo "<div class='messageServer'>Error 404 en el servidor de Visanet</div>";
				// echo "<div class='text-align: center'><a href='http://localhost:4200/nueva-consulta' class='btn btn-green'>Volver a Nueva Consulta Medica</a></div>";
				echo "<a href='https://www.alo.doctor/web/paciente/dist/nueva-consulta' class='btn'>Volver a Nueva Consulta Medica</a>";
			}else if($answer["errorCode"] == 500){
				echo "<div class='messageServer'>Error 500 en el servidor de Visanet</div>";
				// echo "<div class='text-align: center'><a href='http://localhost:4200/nueva-consulta' class='btn btn-green'>Volver a Nueva Consulta Medica</a></div>";
				echo "<a href='https://www.alo.doctor/web/paciente/dist/nueva-consulta' class='btn'>Volver a Nueva Consulta Medica</a>;";
			}else if($answer["errorCode"] == 0){
				echo "<div class='messageServer'>Peticion exitosa, redirigiendo a pagina de llamada ...</div>";

				// header('Location: ' . "http://localhost:4200/nueva-consulta", true, false ? 301 : 302);
				header('Location: ' . "https://www.alo.doctor/web/paciente/dist/nueva-consulta", true, false ? 301 : 302);
    		exit();
			}

			// echo "<pre>";
			// var_dump($respuesta);
			// echo "<pre>";
		}

	}

	if (isset($_POST['sessionToken'])){
		echo "Configuración del botón<br>";
		echo $merchanId = $_POST['merchantId'];
		$amount = $_POST['amount'];
		echo "<br>";
		echo $sessionToken = $_POST['sessionToken'];
		echo "<br>";
		echo $numorden = contador();
		$formulario="
		<form action=\"boton.php\" method='post'>
		<script src=\"https://static-content.vnforapps.com/v1/js/checkout.js?qa=true\"
		data-sessiontoken=\"$sessionToken\"
		data-merchantid=\"$merchanId\"
		data-buttonsize=\"\"
		data-buttoncolor=\"\"
		data-merchantlogo =\"http://labceperu.com/test/files/logo.png\"
		data-merchantname=\"\"
		data-formbuttoncolor=\"#0A0A2A\"
		data-showamount=\"\"
		data-purchasenumber=\"$numorden\"
		data-amount=\"$amount\"
		data-cardholdername=\"GLEIZER\"
		data-cardholderlastname=\"PANDURO\"
		data-cardholderemail=\"gleizerp@gmail.com\"
		data-usertoken=\"\"
		data-recurrence=\"false\"
		data-frequency=\"Quarterly\"
		data-recurrencetype=\"fixed\"
		data-recurrenceamount=\"200\"
		data-documenttype=\"0\"
		data-documentid=\"\"
		data-beneficiaryid=\"TEST1123\"
		data-productid=\"\"
		data-phone=\"\"
		/></script>
		</form>";
		echo $formulario;
		exit;
	}
	?>
</div>

</body>
</html>
