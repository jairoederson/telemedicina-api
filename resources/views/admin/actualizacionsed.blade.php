...
<section class="content-header section-header">
    <div class="row">
        <div class="col-md-12">
            <h1 class="header-title">Actualización Sed</h1>
        </div>
    </div>
...
<script>
    ...
    function obtenerSed(idsed) {
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/obtenersed') }}",
            dataType: "json",
            data: {
                idsed: idsed,
                '_token': '{{ csrf_token() }}'
            ...
            //alert(JSON.stringify(dataRpta));
            $('#nombre').val(dataRpta.sed.name);
            $('#descripcion').val(dataRpta.sed.description);
            $('#cmbDisease').val(dataRpta.sed.diseas_id);
