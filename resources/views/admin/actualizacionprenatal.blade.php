...
<section class="content-header section-header">
    <div class="row">
        <div class="col-md-12">
            <h1 class="header-title">Actualización Prenatal</h1>
        </div>
    </div>
...
<script>
    ...
    obtenerPrenatal('{!! $idprenatal!!}');
    $("#formRegistrarPrenatal").submit(function () {
        ActualizarPrenatal();
        return false;
    });

    function ActualizarPrenatal() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/modificarprenatal') }}",
            dataType: "json",
            data: {
                prenatal: oprenatal,
                '_token': '{{ csrf_token() }}'
    ...
    function obtenerPrenatal(idprenatal) {
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/obtenerprenatal') }}",
            dataType: "json",
            data: {
           ...
           // alert(JSON.stringify(dataRpta));
            $('#nombre').val(dataRpta.prenatal.name);
            $('#descripcion').val(dataRpta.prenatal.description);
            $('#cmbDisease').val(dataRpta.prenatal.diseas_id);
...
<!--<script type="text/javascript" src="{{ asset('assets/js/frontend/admin/registroPrenatal.js')}}"></script>-->
