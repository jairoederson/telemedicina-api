...
<section class="content-header section-header">
    <div class="row">
        <div class="col-md-12">
            <h1 class="header-title">Actualización Apetitos</h1>
        </div>
    </div>
...
<script>
    ...
    obtenerApetitos('{!! $idapetitos!!}');
    $("#formRegistrarApetitos").submit(function () {
        ActualizarApetitos();
        return false;
    });

    function ActualizarApetitos() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/modificarapetitos') }}",
            dataType: "json",
            data: {
                apetitos: oapetitos,
                '_token': '{{ csrf_token() }}'
    ...
    function obtenerApetitos(idapetitos) {
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/obtenerapetitos') }}",
            dataType: "json",
            data: {
                idapetitos: idapetitos,
                '_token': '{{ csrf_token() }}'
            },
            beforeSend: function (xhr) {
            }
            ...
            //alert(JSON.stringify(dataRpta));
            $('#nombre').val(dataRpta.apetitos.name);
            $('#descripcion').val(dataRpta.apetitos.description);
            $('#cmbDisease').val(dataRpta.apetitos.diseas_id);
...
<!--<script type="text/javascript" src="{{ asset('assets/js/frontend/admin/registroApetitos.js')}}"></script>-->
