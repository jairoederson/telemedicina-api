...
<section class="content-header section-header">
    <div class="row">
        <div class="col-md-12">
            <h1 class="header-title">Actualización Deposiciones</h1>
        </div>
    </div>
...
<section class="content paddingleft_right15">
    <div class="row" style="margin-right: -30px;margin-left: -30px">
        <div class="panel panel-primary ">
            <form id="formRegistrarDeposiciones">
                <fieldset>
                    <div class="col-md-12">
                        <div class="col-md-12">
...
<script>
    ...
    obtenerDeposiciones('{!! $iddeposiciones!!}');
    $("#formRegistrarDeposiciones").submit(function () {
        ActualizarDeposiciones();
        return false;
    });

    function ActualizarDeposiciones() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/modificardeposiciones') }}",
            dataType: "json",
            data: {
    ...
    function obtenerDeposiciones(iddeposiciones) {
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/obtenerdeposiciones') }}",
            dataType: "json",
            data: {
                iddeposiciones: iddeposiciones,
                '_token': '{{ csrf_token() }}'
            },
            beforeSend: function (xhr) {
            }
            ...
            //alert(JSON.stringify(dataRpta));
            $('#nombre').val(dataRpta.deposiciones.name);
            $('#descripcion').val(dataRpta.deposiciones.description);
            $('#cmbDisease').val(dataRpta.deposiciones.diseas_id);
...
<!--<script type="text/javascript" src="{{ asset('assets/js/frontend/admin/registroDeposiciones.js')}}"></script>-->
