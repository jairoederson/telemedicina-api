...
<section class="content-header section-header">
    <div class="row">
        <div class="col-md-12">
            <h1 class="header-title">Actualización Orinas</h1>
        </div>
    </div>
...
<script>
    ...
    obtenerOrinas('{!! $idorinas!!}');
    $("#formRegistrarOrinas").submit(function () {
        ActualizarOrinas();
        return false;
    });

    function ActualizarOrinas() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/modificarorinas') }}",
            dataType: "json",
            data: {
                orinas: oorinas,
                '_token': '{{ csrf_token() }}'
    ...
    function obtenerOrinas(idorinas) {
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/obtenerorinas') }}",
            dataType: "json",
            data: {
                idorinas: idorinas,
                '_token': '{{ csrf_token() }}'
            ...
            //alert(JSON.stringify(dataRpta));
            $('#nombre').val(dataRpta.orinas.name);
            $('#descripcion').val(dataRpta.orinas.description);
            $('#cmbDisease').val(dataRpta.orinas.diseas_id);
...
<!--<script type="text/javascript" src="{{ asset('assets/js/frontend/admin/registroOrinas.js')}}"></script>-->
