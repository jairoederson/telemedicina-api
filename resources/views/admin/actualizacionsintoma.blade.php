...
<section class="content-header section-header">
    <div class="row">
        <div class="col-md-12">
            <h1 class="header-title">Actualización Sintomas</h1>
        </div>
    </div>
                ...
                <fieldset>
                    <div class="col-md-12">
                        <div class="col-md-12">
                            ...
                            <div class="form-group label-floating is-empty">
                                <select id="cmbDisease" class="form-control " style="font-size: 14px !important" required>
...
<script>
    ...
    var listarenfermedad = "{{ URL::to('admin/listarenfermedad') }}";
    obtenerSintoma('{!! $idsintomas!!}');
    $("#formRegistrarSintoma").submit(function () {
        ActualizarSintoma();
        return false;
    });

    function ActualizarSintoma() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/modificarsintoma') }}",
            dataType: "json",
            data: {
                sintoma: osintoma,
                '_token': '{{ csrf_token() }}'
    ...
    function obtenerSintoma(idsintoma) {
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/obtenersintoma') }}",
            dataType: "json",
            data: {
                idsintoma: idsintoma,
                '_token': '{{ csrf_token() }}'
            ...
            //alert(JSON.stringify(dataRpta));
            listarEnfermedad(dataRpta.sintoma.diseas_id);
            $('#nombre').val(dataRpta.sintoma.name);
            $('#descripcion').val(dataRpta.sintoma.description);
            $('#cmbDisease').val(dataRpta.sintoma.diseas_id);
...
<!--<script type="text/javascript" src="{{ asset('assets/js/frontend/admin/registroSintoma.js')}}"></script>-->
