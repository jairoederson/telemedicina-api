...
<section class="content-header section-header">
    <div class="row">
        <div class="col-md-12">
            <h1 class="header-title">Actualización Habitos</h1>
        </div>
    </div>
...
<script>
    ...
    obtenerHabitos('{!! $idhabitos!!}');
    $("#formRegistrarHabitos").submit(function () {
        ActualizarHabitos();
        return false;
    });
    ...
    function ActualizarHabitos() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/modificarhabitos') }}",
            dataType: "json",
            data: {
                habitos: ohabitos,
                '_token': '{{ csrf_token() }}'
    ...
    function obtenerHabitos(idhabitos) {
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/obtenerhabitos') }}",
            dataType: "json",
            data: {
                idhabitos: idhabitos,
                '_token': '{{ csrf_token() }}'
            ...
            //alert(JSON.stringify(dataRpta));
            $('#nombre').val(dataRpta.habitos.name);
            $('#descripcion').val(dataRpta.habitos.description);
            $('#cmbDisease').val(dataRpta.habitos.diseas_id);
...
<!--<script type="text/javascript" src="{{ asset('assets/js/frontend/admin/registroHabitos.js')}}"></script>-->
