...
<section class="content-header section-header">
    <div class="row">
        <div class="col-md-12">
            <h1 class="header-title">Actualización Sueños</h1>
        </div>
    </div>
...
<script>
    ...
    obtenerSuenos('{!! $idsuenos!!}');
    $("#formRegistrarSuenos").submit(function () {
        ActualizarSuenos();
        return false;
    });

    function ActualizarSuenos() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/modificarsuenos') }}",
            dataType: "json",
            data: {
                suenos: osuenos,
                '_token': '{{ csrf_token() }}'
    ...
    function obtenerSuenos(idsuenos) {
        $.ajax({
            type: 'post',
            url: "{{ URL::to('admin/obtenersuenos') }}",
            dataType: "json",
            data: {
                idsuenos: idsuenos,
                '_token': '{{ csrf_token() }}'
            ...
            //alert(JSON.stringify(dataRpta));
            $('#nombre').val(dataRpta.suenos.name);
            $('#descripcion').val(dataRpta.suenos.description);
            $('#cmbDisease').val(dataRpta.suenos.diseas_id);
...
<!--<script type="text/javascript" src="{{ asset('assets/js/frontend/admin/registroSuenos.js')}}"></script>-->
